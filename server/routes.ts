...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/widgets", tempAuthMiddleware, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
  ...
  app.put("/api/widgets/:id", tempAuthMiddleware, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
      ...
      const widget = await storage.updateWidget(widgetId, req.body);
  ...
  app.delete("/api/widgets/:id", tempAuthMiddleware, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
      ...
      const widgetId = parseInt(req.params.id);
      await storage.deleteWidget(widgetId, userId);
      res.status(204).send();
    } catch (error) {
  ...
  app.post("/api/widgets/reorder", tempAuthMiddleware, async (req, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
      ...
      await storage.reorderWidgets(userId, widgetIds);
      res.status(200).json({ message: "Widgets reordered successfully" });
    } catch (error) {
